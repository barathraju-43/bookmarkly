...
- If a feature mutates protected rows under Supabase RLS, make sure the table has an explicit `UPDATE` policy as well as `SELECT`, `INSERT`, and `DELETE`. Edit flows can pass locally in mocked tests and still fail in production with an opaque server error if the policy set is incomplete.
